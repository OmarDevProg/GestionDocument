...
<div class="flex h-screen overflow-hidden">
        ...
        <div class="flex flex-col w-64 bg-gray-900 text-white">
            ...
            <div class="flex flex-col flex-grow overflow-y-auto">
                ...
                <nav class="flex-1 px-2 space-y-1">
                    ...
                    <!-- Historique -->
                    <div class="relative">
                        <button id="historyBtn" class="w-full flex items-center px-2 py-2 text-sm font-medium rounded-md bg-gray-700  group">
                        ...
                        <div id="historySubmenu" class="submenu ml-6 mt-1">
                            ...
                            <a href="aHis.php" class="actions-history-link flex items-center px-2 py-2 text-sm rounded-md bg-gray-700 text-gray-300 hover:text-white">
    ...
    <div class="flex-1 flex flex-col overflow-hidden bg-white">
        ...
        $pdo = require 'backend/connect.php';

        /* 1️⃣  Fetch full login history (join with users) */
     $history = $pdo->query("
    SELECT ah.*, u.firstname, u.lastname, f.filename 
    FROM action_history ah
    LEFT JOIN users u ON ah.user_id = u.id
    LEFT JOIN files f ON ah.file_id = f.id
    ORDER BY ah.action_time DESC
")->fetchAll();
?>
        <!-- Tableau de Connexion -->
        <div class="max-w-6xl mx-auto mt-10">
            <h1 class="text-3xl font-bold mb-6">Historique des actions</h1>
            ...
            <div class="bg-white shadow-md rounded-lg overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left">Fichier/Dossier</th>
                    ...
                    <?php foreach ($history as $h): ?>
                        <tr>
                            <td class="px-4 py-2"><?= htmlspecialchars($h['firstname'] . ' ' . $h['lastname']) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($h['action_type']) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($h['filename'] ?? '-') ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($h['description']) ?></td>
                            <td class="px-4 py-2"><?= htmlspecialchars($h['action_time']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
                </table>
